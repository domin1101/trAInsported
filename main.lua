...
package.path = "Scripts/?.lua;External/?.lua;" .. package.path
print(package.path)
DEBUG = false
...
evolution = require("Scripts/evolution")
...
function newGame()
	...
	setupMatch(width, height, 1, 5 * 60, gameMode, chosenAIs, "Suburban", matchIndividuals)
end
...
if DEDICATED then
	...
	function love.load(args)
		...
		AI_DIRECTORY = love.filesystem.getWorkingDirectory() .. "/AI2/"
	...
	function love.update()
		...
		if map.generating() then
			map.generate(nil, nil, nil, nil, matchIndividuals)
		end
		dt = 1/15 --love.timer.getDelta()
		counter = counter + 1
		...
		countSeconds = countSeconds + dt*timeFactor
		...
		if not roundEnded and curMap then
			...
			timeUntilMatchEnd = timeUntilMatchEnd - dt*timeFactor
		...
		elseif not map.generating() then
			...
			if timeUntilMatchEnd > 0 then		--wait until the actual match time is over:
				...
				timeUntilNextMatch = timeUntilNextMatch - dt*timeFactor

				-- display time until next match:
				--io.write( "Starting next match in " .. timeUntilNextMatch .. " seconds.","\r")
				...
				if timeUntilNextMatch <= 0 then
					...
					timeUntilNextMatch = 0--TIME_BETWEEN_MATCHES
	...
	function love.update(dt)
		...
		--end,
		if roundEnded and not map.generating() then
